...
- **Step 3 – Track state**: `message_status.txt` (updated automatically whenever `send_message.sh` runs) summarizes counts, last subjects, and “Waiting on” status. Before planning, check it; once you resolve a request, run `./scripts/mark_messages_read.sh <your-server>` so the summary remains accurate. Ensure `message_status.txt` https://github.com/alexandremattioli/Build/blob/main/message_status.txt is updated with every message you send.
