...
# Management user name to create/use:
$UserNameDefault = 'buildadmin'

# Your SSH PUBLIC key (provided by you) — do not paste private keys.
$PublicKeyDefault = @'
ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEtbG77gQtb713TkgLHno84dAtdJeRimI+44mRw28IbB alexandre@mattioli.co.uk
'@

# Keep password auth during first run for safety; set to $true to harden immediately.
$DisablePasswordAuthDefault = $false

# Enable WinRM over HTTPS (5986) for PowerShell remoting:
$EnableWinRmHttpsDefault = $true

# Enable RDP:
$EnableRdpDefault = $false

# Base directory for heartbeat/logs:
...
function Ensure-LocalUser {
  ...
  if (-not $user) {
    # Prompt for a password once if user doesn't exist (safe default)
...
function Install-AuthorizedKey {
  ...
  if (Test-Path $auth) {
    ...
    $pubLine | Out-File -FilePath $auth -Encoding ascii
  }
...
function Ensure-WinRM-HTTPS {
  ...
  if (-not (Get-NetFirewallRule -DisplayName 'WinRM (HTTPS-In)' -ErrorAction SilentlyContinue)) {
    New-NetFirewallRule -Name 'WinRM-HTTPS-In' -DisplayName 'WinRM (HTTPS-In)' -Direction Inbound -Protocol TCP -Action Allow -LocalPort 5986 | Out-Null
  }
...
function Install-Heartbeat {
  ...
  $opsDir = Join-Path $BaseDir 'CopilotOps'
  $binDir = Join-Path $opsDir 'bin'
...
$line = [pscustomobject]@{
  ...
  IP          = $ip
  UptimeMin   = [int]$uptime.TotalMinutes
  CPU         = [int]$cpu
  MemUsedGB   = $memUsedGB
  MemTotGB    = $memTotalGB
...
function Disable-OpenSSH {
  ...
  if (Get-NetFirewallRule -DisplayName 'OpenSSH Server (sshd)' -ErrorAction SilentlyContinue) {
    Get-NetFirewallRule -DisplayName 'OpenSSH Server (sshd)' | Disable-NetFirewallRule | Out-Null
  }
}

function Remove-WinRMHttps {
  try {
    $listeners = (winrm enumerate winrm/config/Listener) 2>$null
    if ($listeners) {
      $https = $listeners | Select-String 'Listener' -Context 0,4 | ForEach-Object {
        if ($_.Context.PostContext -match 'Transport = HTTPS') { $_ }
      }
  ...
  } catch { Write-Host "WinRM cleanup warning: $_" -ForegroundColor Yellow }

  if (Get-NetFirewallRule -DisplayName 'WinRM (HTTPS-In)' -ErrorAction SilentlyContinue) {
    Get-NetFirewallRule -DisplayName 'WinRM (HTTPS-In)' | Disable-NetFirewallRule | Out-Null
  }
}

function Remove-Heartbeat {
  $task = Get-ScheduledTask -TaskName 'CopilotHeartbeat' -ErrorAction SilentlyContinue
  if ($task) { Unregister-ScheduledTask -TaskName 'CopilotHeartbeat' -Confirm:$false }
  $opsDir = Join-Path $BaseDirDefault 'CopilotOps'
  if (Test-Path $opsDir) { Remove-Item -Recurse -Force $opsDir }
}

function Uninstall-All {
  param([string]$UserName,[switch]$RemoveUser)
  Disable-OpenSSH
  Remove-WinRMHttps
  Remove-Heartbeat
  if ($RemoveUser -and $UserName) {
    try {
      Remove-LocalGroupMember -Group 'Administrators' -Member $UserName -ErrorAction SilentlyContinue
      Remove-LocalUser -Name $UserName -ErrorAction SilentlyContinue
    } catch { Write-Host "User cleanup warning: $_" -ForegroundColor Yellow }
  }
...
try {
  ...
  if ($Uninstall) {
    Uninstall-All -UserName $UserNameDefault -RemoveUser:$RemoveUser
    return
  }
  ...
  Write-Host "Test SSH from your laptop:"
  Write-Host ("  ssh {0}@{1}" -f $UserNameDefault, (hostname))
