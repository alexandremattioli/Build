...
################################################################################
# Script: heartbeat.sh
# Purpose: Send heartbeat for this server
...
push_with_retry() {
   ...
   [ -f scripts/log_error.sh ] && bash scripts/log_error.sh "heartbeat" "git push failed after $max_attempts attempts"
...
# Validate heartbeat JSON (best-effort)
